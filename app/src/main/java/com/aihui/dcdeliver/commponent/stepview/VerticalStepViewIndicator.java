package com.aihui.dcdeliver.commponent.stepview;
...
public class VerticalStepViewIndicator extends View {
    ...
    private float mCompleteCircleRadius;//已完成的圆的半径 区分对待  definition circle radius
    ...
    private void init() {
        ...
        //已完成的圆的半径
        mCompleteCircleRadius = 0.08f * defaultStepIndicatorNum;


        //线与线之间的间距    set mLinePadding
        mLinePadding = 0.85f * defaultStepIndicatorNum;

        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.radiob_blue);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.mipmap.ic_transing);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.mipmap.ic_ok);//未完成的icon
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i < mComplectingPosition)//判断在完成之前的所有点
            {
                ...
                if (mIsReverseDraw) {
                    canvas.drawRect(mLeftY, afterComplectedXPosition + mCompleteCircleRadius - 10, mRightY, preComplectedXPosition - mCompleteCircleRadius + 10, mCompletedPaint);
                } else {
                    canvas.drawRect(mLeftY, preComplectedXPosition + mCompleteCircleRadius - 10, mRightY, afterComplectedXPosition - mCompleteCircleRadius + 10, mCompletedPaint);
                }
            } else {
               //if (mIsReverseDraw) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
            ...
            if (i < mComplectingPosition) {
                Rect completeRec = new Rect((int) (mCenterX - mCompleteCircleRadius), (int) (currentComplectedXPosition - mCompleteCircleRadius), (int) (mCenterX + mCompleteCircleRadius), (int) (currentComplectedXPosition + mCompleteCircleRadius));
                mCompleteIcon.setBounds(completeRec);
